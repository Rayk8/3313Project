...
// **Search button handler** (Key fix: overwrite the main list)
document.getElementById("searchButton").addEventListener("click", function() {
  const query = document.getElementById("searchQuery").value;
  fetch(`http://localhost:8080/search?query=${encodeURIComponent(query)}`)
    .then(response => response.text())
    .then(data => {
      // Instead of appending at the bottom, we replace the main book list.
      document.getElementById("availableBooksList").innerHTML = data;
    })
    .catch(error => {
